...
predict_data_SMAPE = function(df, params){
  ...
  new_df <- data.frame(day = df$day, pred_I_SMAPE = pred_df[4], pred_R_SMAPE = pred_df[5])
...
israel_pred_df = function(){
  ...
  a_SSE = predict_data_SSE(df1, starting_param_val)
  df1_SSE <- a_SSE[[1]]
  df1_params <- a[[2]]
  k = a[[3]]
  
  df1 <- cbind(df1, df1_SMAPE, df1_SSE)
  ...
  df2_SMAPE = a[[1]]
  a_SSE = predict_data_SSE(df2, df1_params)
  df2_SSE <- a_SSE[[1]]
  df2_params <- a[[2]]
  k = a[[3]]
  df2 <- cbind(df2, df2_SMAPE, df2_SSE)
  ...
  df3_SMAPE = a[[1]]
  a_SSE = predict_data_SSE(df3, df2_params)
  df3_SSE <- a_SSE[[1]]
  df3_params <- a[[2]]
  k = a[[3]]
  df3 <- cbind(df3, df3_SMAPE, df3_SSE)
  ...
  df4_SMAPE = a[[1]]
  a_SSE = predict_data_SSE(df4, df3_params)
  df4_SSE <- a_SSE[[1]]
  df4_params <- a[[2]]
  k = a[[3]]
  df4 <- cbind(df4, df4_SMAPE, df4_SSE)
