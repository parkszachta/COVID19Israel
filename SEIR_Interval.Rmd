...
pred_df2 = sir_1(beta = exp(pars_df2[1]), gamma = exp(pars_df2[2]), I0 = df1$I[nrow(df1)],
                    R0 = df1$R[nrow(df1)], times = df2$day, N = N, lambda = lambda,
                    mu = mu) 
...
pred_df3 = sir_1(beta = exp(pars_df3[1]), gamma = exp(pars_df3[2]), I0 = df2$I[nrow(df2)],
                    R0 = df2$R[nrow(df2)], times = df3$day, N = N, lambda = lambda,
                    mu = mu) 
...
pred_df4 = sir_1(beta = exp(pars_df4[1]), gamma = exp(pars_df4[2]), I0 = df3$I[nrow(df3)],
                    R0 = df3$R[nrow(df3)], times = df4$day, N = N, lambda = lambda,
                    mu = mu) 
...
p2 = base +
   geom_smooth(mapping = aes(x = date, y = pred_R$R, color = colour),
             data = israel_new, size = 1,color = mn) +
